...
/**
    ...
    alpha values. Some image editing applications don't save proper alpha values, as explained at PRREImageManager::createFromFile().
    For bitmaps not having proper alpha bits use (PRRE_SRC_ALPHA, PRRE_ONE) for src and dst respectively.
    For bitmaps having proper alpha bits use (PRRE_SRC_ALPHA, PRRE_ONE_MINUS_SRC_ALPHA) for src and dst respectively to utilize real alpha.
