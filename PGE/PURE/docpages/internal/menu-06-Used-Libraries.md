...
My selected GUI library TODO add link is Dear ImGui, it officially has support for SDL and GLFW, but not for SFML.  
There is an [unofficial repo adding SFML support to Dear ImGui though](https://github.com/SFML/imgui-sfml).  
Note: if you use [TGUI](https://github.com/texus/TGUI/) then it has support for all 3 though.  
...
For DDS support, it is ok if a given lib does not support resizing because DDS stores different MIP map levels, so downscaling can be done by simply selecting a MIP map as base texture, instead of actually resizing.  
...
PURE v0.4 has its own OBJ model file parser implementation, however that is an improved version of the legacy PR00FPS (2007) implementation.  
It still does NOT support MTL file format which is a very fundamental addition to OBJ, since OBJ can reference MTL file, to describe material data.  
...
\section pure_used_libs_lights Lights
...
\section pure_used_libs_state_cache State Caching
